...
`protected events: IEvents` — брокер событий, через который модель уведомляет Презентер об изменениях.
...
После установки эмитит событие `product:selected` с payload `{ item: this.selectedItem }`, чтобы Презентер открыл модалку с этим товаром.
...
- модель корзины. Хранит товары, считает сумму, защищает от дублей. Уведомляет Презентер через события при любом изменении состава корзины.

**Конструктор класса:**  
`constructor(protected events: IEvents) {}`  
Принимает один параметр — `events: IEvents` (брокер событий). Сохраняет его в защищённое поле `protected events` для генерации событий при изменении состава корзины.  
Поле `items` инициализируется пустым массивом при объявлении (корзина изначально пуста).
...
`addItem(item: IProduct): void` — добавление товара в массив корзины. Принимает в качестве параметра объект товара. Предотвращает дублирование по id: проверяет наличие товара по `hasItem()`, если товара нет — добавляет его.  
После добавления эмитит событие `cart:changed` через вспомогательный метод `emitChange()`.

`removeItem(id: string): void` — удаление товара из корзины. Принимает в качестве параметра строку с идентификатором товара. Использует `filter`, чтобы оставить все товары, кроме удаляемого.  
После удаления эмитит событие `cart:changed` через вспомогательный метод `emitChange()`.

`clear(): void` — полностью очищает корзину (используется после успешного оформления заказа). Устанавливает `items = []`.  
После очистки эмитит событие `cart:changed` через вспомогательный метод `emitChange()`.

`getTotalPrice(): number` — получение стоимости всех товаров в корзине. Возвращает число (сумма цен). Корректно обрабатывает товары с `price: null`, считая их стоимость как 0 (через оператор `??`).

`getCount(): number` — получение количества товаров в корзине (для счётчика на иконке корзины). Возвращает целое число.

`hasItem(id: string): boolean` — проверка наличия товара в корзине по его id. Принимает в качестве параметра строку с идентификатором товара. Возвращает `true` — если товар есть, `false` — если нет. Использует метод `some()` для проверки.
...
- модель покупателя. Хранит данные, частично обновляет их, валидирует. Уведомляет Презентер через события при изменении данных или после валидации.

**Конструктор класса:**  
`constructor(protected events: IEvents) {}`  
Принимает один параметр — `events: IEvents` (брокер событий). Сохраняет его в защищённое поле `protected events` для генерации событий при изменении данных или валидации.  
Начальное состояние объекта `data` задаётся при объявлении поля. Значение по умолчанию для способа оплаты — пустая строка (`payment: ''`), чтобы соответствовать состоянию UI, где кнопки выбора оплаты изначально неактивны. Остальные поля инициализируются пустыми строками. Сброс до этого состояния реализован отдельным методом `clear()`.
...
После сохранения эмитит событие `buyer:changed` с указанием изменённого поля и его значения (через `Object.keys` / `Object.values`), чтобы Презентер мог запустить валидацию или обновить состояние формы.
...
- слушатель на кнопки оплаты order:payment:change
...
Взаимодействие между слоями Model, View и Presenter осуществляется через брокер событий (EventEmitter). Каждое действие пользователя генерирует событие с определённым именем и payload'ом. Презентер подписывается на эти события и обрабатывает их.

События могут генерироваться как представлениями (View) — в ответ на действия пользователя, — так и моделями данных (Model) — при изменении их внутреннего состояния.
...
#### События от Моделей (Model)

События генерируются моделями при изменении их внутреннего состояния. Презентер подписывается на них и обновляет представления.

`products:changed`  
Генерируется: **Products** — после вызова `setItems()`  
Payload: `{ items: IProduct[] }`  
Действие Презентера: обновляет каталог товаров на главной странице.

`product:selected`  
Генерируется: **Products** — после вызова `setSelectedItem()`  
Payload: `{ item: IProduct | null }`  
Действие Презентера: открывает или закрывает модальное окно с детальным просмотром товара.

`cart:changed`  
Генерируется: **Cart** — после вызова `addItem()`, `removeItem()` или `clear()`  
...
Действие Презентера: отображает ошибки под соответствующими полями формы или разблокирует кнопку отправки, если ошибок нет.

#### Действия Презентера по событиям

`product:open` — Открыть модальное окно с детальным просмотром товара.  
Действие Презентера: получает товар из Products, создаёт CardPreview, вставляет в Modal и открывает.

`basket:open` — Открыть модальное окно корзины.  
Действие Презентера: получает товары из Cart, создаёт CardBasket для каждого, вставляет в Basket, затем в Modal и открывает.

`basket:order` — Открыть форму заказа (первый шаг).  
Действие Презентера: создаёт OrderForm, вставляет в Modal и открывает.

`order:submit` — Перейти ко второму шагу оформления.  
Действие Презентера: создаёт ContactsForm, вставляет в Modal и открывает.
