...
Конструктор класса:
`constructor()` - не принимает параметров. Инициализирует внутренние поля пустыми значениями.

Поля класса:
`private items: IProduct[]` - хранит массив всех товаров.
`private selectedItem: IProduct | null` - хранит товар, выбранный для подробного отображения (по умолчанию пустой) (значение null - значит модальное окно закрыто).
...
`export class Cart {}` - Класс отвечает за управление списком товаров в корзине, подсчет количества и общей стоиомости.

Конструктор класса:
`constructor()` - не принимает параметров. Инициализирует корзину пустым массивом товаров.
...
Конструктор класса:
`constructor()` - не принимает параметров. Инициализирует объект покупателя пустыми строками и значением по умолчанию для способа оплаты.

Поля класса:
`private data: IBuyer` - объект с текущими данными покупателя.
...
`validate() : Record<keyof IBuyer, string> | {}` - выполняет валидацию полей (поле валидно если не пустое). Метод создает и возвращает объект с ошибками вида `{payment: 'Не выбран тип оплаты', email: 'Укажите email'}`, где ключи - названия полей IBuyer. Пустой объект {} - если все поля валидны. Проверяет каждое поле на непустоту и формирует сообщения об ошибках по ключам интерфейса `IBuyer`.
